<?php
	...
	$sql = "SELECT DISTINCT version,deployed_date,modified_date FROM reports_tbl ORDER BY id DESC";// LIMIT 3 OFFSET 0";
	...
	if (mysqli_num_rows($result) > 0) {
	  ...
	  while($row = mysqli_fetch_assoc($result)) {
	  	if($row["modified_date"]==NULL){
	  		...
	  		$concatsqldate="AND deployed_date='{$row["deployed_date"]}' AND modified_date is NULL";
	  	}else{
	  		$date=date_create($row["modified_date"]);
	  		$concatsqldate="AND modified_date='{$row["modified_date"]}'"; 
	  	}
		$created_date = date_format($date,"M d, Y");
		
	  	echo "<ul style='list-style-type:circle';>
	  			<li><h4 >
	  				<a>v".$row["version"]. " (" . $created_date . ")</a>
	  				</h4>";
